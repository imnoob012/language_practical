...
public class ViewController {
	...
	public String getAllTableData(Model model) {
		...
		// ユーザーテーブルを取得
		UserListDto userList = userSkillService.findAllOrSearch(null);
		// ページに表示させる名前とスキルを取得
		SkillListDto skillList = userSkillService.findAllOrSearchSkills(null);
		
		data.setUserIds(userList.getUserIds());
		data.setUserNames(userList.getUserNames());
		data.setRoles(userList.getRoles());
